-- ============================================================================
-- doctorVet — 0019: fix accept_invite() — profiles has `full_name`, not `name`.
--
-- The 0017 version read `coalesce(name, email)` from `profiles`, but that table's
-- column is `full_name`. So accept_invite() raised `column "name" does not exist`
-- and the invitee saw a red error instead of joining. This replaces the function
-- (same logic, correct column) and reloads the PostgREST schema cache.
--
-- Additive & safe: only redefines the function. Apply AFTER 0017.
-- ============================================================================
...
begin
  ...
  -- profiles stores the display name in `full_name` (not `name`).
...
-- Make the corrected function visible to the API immediately.
