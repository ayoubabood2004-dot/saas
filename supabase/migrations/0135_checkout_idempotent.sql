-- ============================================================================
-- doctorVet — 0135: البيعة تُعاد بأمان — مرجعُ عميلٍ يمنع الفاتورة المكرّرة
--
-- ── الخطر القائم اليوم (بلا أي طابور) ─────────────────────────────────────
-- الكاشير يضغط «إصدار الفاتورة». الطلب يصل الخادم، والخادم يسجّل البيعة
-- ويخصم المخزون… ثم ينقطع النت قبل أن يعود الجواب. الشاشة تقول «فشل».
-- فيعيد الكاشير — وتُسجَّل **فاتورةٌ ثانية**، ويُخصم المخزون **مرّتين**.
--
-- لا شيء يمنع هذا اليوم. ومعرّف الفاتورة يولَد بالخادم، فليس للعميل ما يقول
-- به «هذه هي نفسها البيعة التي أرسلتها قبل قليل».
--
-- والطابور (outbox) يعيد المحاولة بطبعه، فلا يجوز أن يشمل البيع قبل هذا —
-- وإلا حوّلنا فشلاً نادراً إلى ازدواجٍ منهجيّ.
--
-- ── الحلّ: نفس مبدأ الطابور القائم ────────────────────────────────────────
-- الطابور يحمي إدراج المنتجات بـ`upsert` على معرّفٍ **يولده الجهاز**: لو وصل
-- الطلب الأول فعلاً، لا يُزرع الصفّ مرّتين. نعطي البيعة المِثل: العميل يولّد
-- `client_ref` مرّةً واحدة لكل محاولة بيع، ويرسله مع كل إعادة. والدالّة:
--
--   * تبحث عنه أولاً — إن وُجد، تُرجع الفاتورة القديمة **بلا أي كتابة**،
--     فلا فاتورةَ ثانية ولا خصمَ ثانٍ.
--   * وإن لم يوجد، تسجّل وتختم الفاتورة بالمرجع.
--
-- ── والسباق كذلك ──────────────────────────────────────────────────────────
-- نسختان من نفس البيعة قد تصلان معاً، فتتجاوزان البحث كلتاهما. الفهرس الفريد
-- يوقف الثانية، وكتلةُ الاستثناء تلتقط الخرق وتُرجع فاتورة الأولى. وبما أن
-- الكتلة معاملةٌ فرعية، فما كتبته الثانية — بما فيه خصمُ المخزون — يتراجع
-- كاملاً. فالنتيجة واحدة سواء تسلسلت النسختان أو تزامنتا.
--
-- ── وما يبقى كما هو ───────────────────────────────────────────────────────
-- المرجع **اختياريّ**: نداءٌ بلا `client_ref` يسلك الطريق القديم حرفاً بحرف.
-- فالنسخ القديمة من التطبيق تبقى تعمل، والفهرس جزئيّ (`where not null`) فلا
-- يمسّ الفواتير الـ3204 القائمة.
--
-- تراجع:
--   drop index if exists invoices_client_ref_idx;
--   alter table invoices drop column if exists client_ref;
--   (وأعد تعريف retail_checkout من 0122)
-- ============================================================================

alter table invoices add column if not exists client_ref text;

-- فريدٌ داخل العيادة الواحدة، وجزئيّ كي لا يقيّد الصفوف القديمة (client_ref
-- فارغ فيها كلها) ولا يشغل مساحةً لها.
create unique index if not exists invoices_client_ref_idx
  on invoices (clinic_id, client_ref) where client_ref is not null;
...
begin
  ...
  begin
    insert into invoices (clinic_id, subtotal, discount, discount_type, total, amount_paid, cost_total, profit,
                          ...
                          staff_id, notes, status, client_ref)
    values (v_clinic, v_subtotal, v_discount, v_dtype, v_total, v_paid, v_cost, v_total - v_cost,
            ...
            nullif(p_meta->>'staff_id','')::uuid, nullif(p_meta->>'notes',''), 'paid', v_ref)
