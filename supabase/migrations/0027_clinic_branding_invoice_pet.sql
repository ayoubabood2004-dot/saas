-- ============================================================================
-- doctorVet — 0027: clinic branding (logo + socials) and invoice patient name.
--
--  • clinic_prefs gains logo_url (a compressed data-URL, like pet avatars) plus
--    social_facebook / social_instagram handles — shown on printed invoices.
--  • invoices gains pet_name so a sale raised for a specific animal prints the
--    patient's name alongside the owner.
--  • retail_checkout now persists pet_name from the sale meta.
-- Clinic-isolated by the existing RLS. Additive & idempotent. Apply AFTER 0026.
-- ============================================================================
...
-- Redefine retail_checkout to also store the (optional) pet name from p_meta.
...
begin
  ...
  insert into invoices (clinic_id, subtotal, discount, discount_type, total, cost_total, profit,
                        item_count, customer_name, customer_phone, pet_name, payment_method, status)
  values (v_clinic, v_subtotal, v_discount, v_dtype, v_total, v_cost, v_total - v_cost,
          ...
          nullif(p_meta->>'pet_name',''), nullif(p_meta->>'payment_method',''), 'paid')
