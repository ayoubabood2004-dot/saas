 ...
 *
 * **ترمي ولا تُرجع قائمةً فارغة.** `waMsgs` نطاقٌ باردٌ (يصل مع نصف القاموس
 * البارد، `ensureDictionary`)، و`i18n.t` على مصفوفةٍ لم تصل تُرجع المفتاحَ نصّاً
 * لا مصفوفة. وكانت هنا تُرجع `[]` بصمت — قائمةٌ ناقصةٌ أخطرُ من خطأ ظاهر: صفرُ
 * صياغاتٍ يعني رسالةً فارغةً تُصاغ لزبون العيادة، ولا أحدَ يرى السبب. والصفحاتُ
 * كلُّها تنتظر القاموسَ قبل أن تُرسم (`page()`)، فالرميةُ لا تُبلغ إلا إن انكسرت
 * البوّابة — وحينها تُقال بتوست «أعد المحاولة» لا برسالةٍ فارغة. */
export function waVariants(pool: WaPool): string[] {
  const v = i18n.t(`waMsgs.${pool}`, { returnObjects: true }) as unknown;
  if (Array.isArray(v)) return v as string[];
  throw new Error(`waMsgs.${pool}: dictionary not loaded`);
}
