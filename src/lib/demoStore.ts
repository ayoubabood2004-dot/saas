...
export function loadDB(): DemoDB {
  ...
  const fresh = seed();
  // **بذرةُ الإقلاع أفضلُ جهدٍ عمداً.** `saveDB` صارت ترمي (أدناه)، و`loadDB`
  // مسارُ **قراءة** ينادى بمئتَي موضع. فلو رمت البذرةُ على حصّةٍ ممتلئة لسقطت
  // كلُّ قراءةٍ بالتطبيق على شاشةٍ بيضاء — والحالُ أنّ الجلسةَ تشتغل بالذاكرة
  // تماماً. الكتابةُ هي التي تُسمَع، لا القراءة.
  try { saveDB(fresh); } catch { /* حصّةٌ ممتلئة: الجلسةُ تكمل بالذاكرة */ }
  return fresh;
}

/** يُرمى حين ترفض حصّةُ الجهاز الكتابة — الشاشةُ تقرأ `name` لا نصّاً عربياً. */
export class DemoQuotaError extends Error {
  constructor() { super("DEMO_QUOTA_FULL"); this.name = "DemoQuotaError"; }
}
...
export function saveDB(db: DemoDB) {
  try {
    ...
    // حصة localStorage امتلأت (~5MB) — الكتابة ضاعت فعلاً.
    //
    // كنّا نُعلن الحدثَ وحدَه فتعرض App تحذيراً. وهذا نصفُ الحلّ: الدالّةُ
    // **ترجع طبيعياً**، فالشاشةُ تكمل مسارَ النجاح — رنّةٌ، و«تمّ»، وتُغلق
    // النافذة، وتُعيد التحميلَ من مخزنٍ لم يتغيّر. فيجتمع تحذيرٌ ونجاحٌ بنفس
    // اللحظة، والصفُّ ببساطة غائب. وأسوأُ منه أنّ التحذيرَ يُعرض **مرّةً واحدة
    // بكلّ تحميلِ صفحة** (`warned.current` بـApp.tsx)، فالضياعُ الثاني وما
    // بعده صامتٌ تماماً.
    //
    // فالآن تُرمى: «الكتابةُ تُسمَع» (CLAUDE.md §٣). والحدثُ يبقى لأنه يحمل
    // **السبب** — الرميةُ تقول «ما انحفظ»، والتوستُ يقول «امتلأت المساحة».
