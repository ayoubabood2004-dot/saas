...
export function describeDbError(e: unknown, t: TFunction): string {
  ...
  // المطابقةُ على **الرمز** أوّلاً والرسالةُ احتياطاً: الرمزُ عقدٌ، والرسالةُ
  // نصٌّ قد يُترجَم أو يُعاد صياغتُه. والاحتياطُ لازمٌ لأن الرمزَ خاصّيةٌ على
  // الكائن تسقط عند عبور JSON (صندوقُ الصادر يخزّن ويعيد).
  if (err.code === "no_row_updated"
      || (typeof err.message === "string" && err.message.includes("no_row_updated"))) {
