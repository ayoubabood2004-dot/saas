...
/* ============================================================================
 * اسمُ المنتج كما **يطابقه الخادم** — مرآةُ `inv_norm_name` بالقاعدة (0117).
 *
 * كان ساكناً بـ`repo.ts` خاصّاً، فلا تراه الشاشةُ التي تقرّر «هذي مادةٌ جديدة».
 * وهو الفرعُ الثاني للمطابقة بـ`record_purchase`: سطرٌ بلا باركود يُطابَق
 * **بالاسم**. فالشاشةُ تقول «جديد» والخادمُ يرصّد على منتجٍ قائم.
 *
 * **وهو ليس `searchable`**: هذه تمسح المسافات كلَّها (`\s+` ⇒ ""), وتلك تطويها
 * إلى مسافةٍ واحدة. فمقارنةُ نيّةِ الشاشة بمفتاحٍ غير مفتاح الخادم تعطي جواباً
 * مختلفاً — ولهذا يسكن هنا بجوار أخواته لا بشاشةٍ ولا بوكيل.
 *
 * تنبيهٌ للتوسعة: القاعدة اليوم **بلا `btrim` وبلا محارفَ خفية**، وهذه مرآتُها
 * بالضبط عدا `trim`. أيُّ توسعةٍ هنا تلزمها هجرةٌ تقابلها وفحصُ تكافؤ — وإلا
 * صار طرفٌ مطبَّعاً وطرفٌ لا، وهو أسوأ من لا تطبيع.
 */
export const invNormName = (v: string | null | undefined): string =>
  (v ?? "")
...
/* ============================================================================
 * مفتاحُ التجميع — اسمُ شركةٍ أو صنفٍ كما **يُقارَن**، لا كما يُكتب.
 *
 * ولماذا يسكن هنا لا بكل شاشة: كان معرَّفاً ثلاثَ مرّات (`normKey` بشاشة
 * المخزون، و`normKey` بشاشة الشراء، و`nameKey` بالكتلوج المبدئيّ) — وحين
 * وُسّع أحدُها لم يُوسَّع الباقي، فصار «حارسُ تكرارٍ» يسمح بـ١٠٢ شركةٍ مكرّرة
 * من أصل ١٤٣. تعريفٌ واحدٌ مصدَّر، ومن أراد المقارنةَ استورده.
 *
 * **والطرفان يمرّان من هنا** — هذه ليست وصيّةً بل ما انكسر حرفياً: كان
 * الطرفُ الأيسر `groupKey(name)` والأيمنُ `typed.toLowerCase()`، فـ«شركة تاج
 * الخيل» تُقارن بـ«شركهتاجالخيل» ولا تتطابقان أبداً. راجع رأس `searchable`
 * أعلاه: الدرسُ كان مكتوباً على بُعد ثلاثة أسطرٍ من موضع كسره.
 * ==========================================================================*/
 ...
 *  `inv_norm_group()` (0196)، ويحرس تطابقَهما `group-key-parity.mjs`. */
export const groupKey = (s: string | null | undefined): string =>
  searchable(normGroupName(s));
