 ...
 * العربية.
 *
 * **وهي اليوم بلا أثرٍ بأيّ مسار** — أقولها صراحةً لأن أوّلَ صياغةٍ لهذا
 * التعليق ادّعت أنها تغطّي ثلاثَ حالاتٍ لا تغطّي واحدةً منها: `vp_lang="ckb"`
 * قيمةٌ مخزّنة فتخرج الدالّةُ فوراً بالسطر الأوّل ولا تعرّب شيئاً · وتخزينٌ
 * محجوب يجعل `initialLang()` تسقط إلى "ar" أصلاً فلا تضيف الدالّةُ شيئاً ·
 * وحفظُ اختيارِ الإنكليزية هو نفسُه الخروجُ المبكّر، يبقى قائماً لو حُذفت
 * الدالّةُ كلُّها. ادّعاءٌ غلط أسوأ من «ما أعرف» — وتعليقٌ كهذا يُصدَّق بعد
 * ستّة أشهر.
 *
 * فلماذا تبقى؟ لأنها تثبّت عقداً لا تغطية: **هاتان الصفحتان عربيّتان مهما
 * كان افتراضُ التطبيق**. الافتراضُ قرارُ منتجٍ قد ينقلب، وهاتان لا تنقلبان
 * معه. والثقبُ الوحيدُ الحقيقيّ الباقي معلومٌ وغيرُ مُصلَح: زائرٌ يحمل
 * `vp_lang` بلغةٍ أخرى يرى نصوصَ `t(` بلغته داخل صفحةٍ عربيةِ الاتجاه —
 * بندٌ مستقلٌّ لا يُحشر هنا.
 *
