  ...
  return (
    <div className="card overflow-hidden p-0">
      <div className="flex items-center gap-2.5 border-b border-line bg-gradient-to-br from-brand-500/10 to-success-500/10 px-4 py-3">
        ...
        <h3 className="font-display font-bold text-ink">المبيعات السابقة</h3>
        {rows.length > 0 && (
          <span className="chip ms-auto bg-surface-1 text-2xs font-bold text-ink-muted tabular-nums" title="إجمالي الإنفاق">{money(totalSpend)}</span>
        )}
      </div>
      ...
      {!loaded ? (
        ...
        <div className="grid place-items-center px-4 py-8 text-center">
          ...
          <p className="text-sm text-ink-subtle">لا توجد مبيعات مسجّلة لهذا الحيوان.</p>
        </div>
      ) : (
        <>
          <ul className={cn("divide-y divide-line", showAll && "max-h-64 overflow-y-auto [scrollbar-width:thin]")}>
            {shown.map(({ inv, names }) => {
              ...
              const summary = names.length ? names.join("، ") : `${inv.item_count} صنف`;
              return (
                <li key={inv.id} className="flex items-center gap-3 px-4 py-2.5">
                  <div className="min-w-0 flex-1">
                    <p className="flex items-center gap-1.5 truncate text-sm font-semibold text-ink">
                      <span className="truncate">{summary}</span>
                      {refunded && <span className="chip shrink-0 inline-flex items-center gap-0.5 bg-danger-50 text-2xs font-medium text-danger-600 dark:bg-danger-500/15 dark:text-danger-300"><RotateCcw size={10} /> مُرتجع</span>}
                    </p>
