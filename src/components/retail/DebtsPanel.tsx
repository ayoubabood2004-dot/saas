...
import {
  ...
  Banknote, CreditCard, ArrowLeftRight, CheckCircle2, Wallet,
} from "lucide-react";
...
/** سجل الديون — every sale still carrying a balance (total_paid < grand_total), with a
 *  "تسديد دفعة" action to record installments until each debt is fully settled. */
export function DebtsPanel({ invoices, onChanged }: { invoices: Invoice[]; clinicId?: string; onChanged: () => void }) {
  ...
  // Only live debts: not refunded, and still owing. Most-recent first.
  const debts = useMemo(() => {
    const ql = q.trim().toLowerCase();
    return invoices
      .filter(isDebt)
      .filter((inv) => !ql || (inv.customer_name ?? "").toLowerCase().includes(ql) || (inv.customer_phone ?? "").includes(ql))
      .sort((a, b) => b.created_at.localeCompare(a.created_at));
  }, [invoices, q]);

  const outstanding = useMemo(() => round2(debts.reduce((s, i) => s + dueOf(i), 0)), [debts]);
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div className="card flex items-center gap-3 p-3.5">
          ...
          <div className="min-w-0">
            <p className="truncate text-lg font-bold text-ink tabular-nums">{String(debts.length)}</p>
            <p className="truncate text-xs text-ink-subtle">{t("retail.debtorsCount", "عدد الفواتير المدينة")}</p>
          </div>
        </div>
      </div>
      ...
      {debts.length === 0 ? (
        <div className="card grid place-items-center p-12 text-center text-ink-subtle">
          ...
          {invoices.filter(isDebt).length === 0 ? t("retail.noDebts", "لا توجد ديون — كل الفواتير مسدّدة بالكامل.") : t("retail.noDebtMatch", "لا توجد ديون مطابقة لبحثك.")}
        </div>
      ) : (
        <motion.div variants={staggerContainer} initial="initial" animate="animate" className="space-y-2">
          {debts.map((inv) => {
            ...
            return (
              <motion.div key={inv.id} variants={staggerItem} className="card flex flex-wrap items-center gap-3 p-3.5">
                <span className="grid h-11 w-11 shrink-0 place-items-center rounded-2xl bg-warn-50 text-warn-600 dark:bg-warn-500/15 dark:text-warn-300"><HandCoins size={20} /></span>
                <div className="min-w-0 flex-1">
                  <p className="flex items-center gap-2 truncate text-sm font-semibold text-ink">
                    <User size={13} className="shrink-0 text-ink-subtle" /> {inv.customer_name || t("retail.walkIn", "عميل نقدي")}
                  ...
                  <div className="flex flex-wrap items-center gap-x-3 gap-y-0.5 text-xs text-ink-subtle">
                    {inv.customer_phone && <span className="flex items-center gap-1" dir="ltr"><Phone size={11} /> {inv.customer_phone}</span>}
                    <span className="flex items-center gap-1"><CalendarClock size={11} /> {formatDate(inv.created_at, i18n.language)}</span>
                  </div>
                  <div className="mt-1 flex flex-wrap items-center gap-x-3 gap-y-0.5 text-xs">
                    <span className="text-ink-muted">{t("retail.grandTotal", "إجمالي الفاتورة")}: <span className="font-semibold tabular-nums text-ink">{money(inv.total)}</span></span>
                    <span className="text-ink-muted">{t("retail.paidSoFar", "المدفوع")}: <span className="font-semibold tabular-nums text-success-600">{money(paidOf(inv))}</span></span>
                  </div>
                </div>
