...
export function StickyNotes() {
  ...
  return (
    <div className="relative px-1 pb-2 pt-4">
      ...
      <div className="absolute inset-x-4 bottom-0 top-6 -rotate-2 rounded-2xl bg-rose-400/90 shadow-card dark:bg-rose-500/70" />
      <div className="absolute inset-x-2 bottom-1 top-5 rotate-[1.6deg] rounded-2xl bg-amber-300 shadow-card dark:bg-amber-400/80" />

      {/* الورقة الخضراء العلوية */}
      <div className="relative rounded-2xl bg-[#b9e489] p-4 shadow-raised dark:bg-[#93c368]">
        {/* الملقط */}
        <Paperclip size={30} strokeWidth={2.5} className="absolute -top-4 start-6 -rotate-[24deg] text-emerald-700 drop-shadow-md dark:text-emerald-900" aria-hidden />
        {/* طية الزاوية */}
        ...
        <div className="flex items-center justify-between gap-1">
          <button
            ...
            className="grid h-9 w-9 shrink-0 place-items-center rounded-full text-emerald-900/70 transition hover:bg-emerald-900/10 hover:text-emerald-950 active:scale-95"
          >
            <PrevIcon size={20} strokeWidth={2.5} />
          </button>
          <div className="min-w-0 text-center">
            <p className="truncate font-display text-sm font-extrabold text-emerald-950">
              {dayDate.toLocaleDateString(lang, { weekday: "long", day: "numeric", month: "long" })}
            </p>
            <div className="mt-0.5 flex items-center justify-center gap-1.5">
              {relLabel && <span className="rounded-full bg-emerald-900/15 px-2 py-0.5 text-2xs font-bold text-emerald-950">{relLabel}</span>}
              {offset !== 0 && (
                <button onClick={() => { playTap(); setOffset(0); }} className="rounded-full bg-white/60 px-2 py-0.5 text-2xs font-bold text-emerald-900 transition hover:bg-white/80">
        ...
        <textarea
          ...
          className="mt-2 block min-h-44 w-full resize-y rounded-lg bg-transparent px-1 text-[15px] leading-7 text-emerald-950 outline-none placeholder:text-emerald-900/45"
          style={{
            backgroundImage: "repeating-linear-gradient(transparent, transparent calc(1.75rem - 1px), rgba(6,78,59,.16) calc(1.75rem - 1px), rgba(6,78,59,.16) 1.75rem)",
            backgroundAttachment: "local",
          }}
        />

        {/* آخر تعديل + حالة الحفظ */}
        <div className="mt-1.5 flex min-h-4 items-center justify-between gap-2 text-2xs font-semibold text-emerald-900/60">
