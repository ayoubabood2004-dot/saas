...
function hardcodedCount(path) {
  ...
  for (const line of src.split("\n")) {
    ...
    /* إعفاءٌ **بسببٍ مكتوب** — نفسُ عُرف `unbounded:` و`swallow-ok:`:
     * `/* i18n-data: <السبب> *\/` لعربيّةٍ **بيانات** لا نصٍّ معروض (جدولُ نقل
     * حروفٍ، قائمةُ كلماتٍ تُطابَق بها مدخلاتُ المستخدم…). ترجمتُها لا معنى
     * لها: هي مفاتيحُ مطابقةٍ لا رسائلُ تُقرأ. والسببُ يبقى مرئياً بالسطر
     * فلا يصير الإعفاءُ باباً خلفياً. */
    if (/\/\*\s*i18n-data:\s*\S/.test(line)) { continue; }
    n++;
  }
  return n;
}
