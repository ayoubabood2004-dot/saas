...
/* سقفُ `main` رُفع مرّةً واحدة: 480,000 ⇒ 482,000 (0193).
 *
 * **القياسُ قبل الرفع، كما يشترط هذا الملفّ.** الزيادةُ ١٬٠٢٢ بايتاً مضغوطة،
 * كلُّها **نصُّ ترجمةٍ** لفترة السحب بلغتين — لا `import` جديدٌ ولا مكتبة.
 * وحُوول تفاديها أوّلاً بتقصير النصوص فنزلت ٩٥ بايتاً وبقيت فوق السقف، وما
 * بقي كان سيقصّر رسائلَ سلامةٍ تُقرأ قبل ذبح قطيع — وذاك ثمنٌ لا يُدفع
 * لميزانيّة.
 *
 * وقياسُ الرفع كشف الجذرَ الحقيقيّ، ويُكتب هنا ليُعالَج لا ليُنسى:
 *   repo-*.js  = ٢٠٣ كيلو مضغوطة بمسار الإقلاع الحرج،
 *   منها ar.json ٧٥ + en.json ٦٦ = **١٤١ كيلو (٦٩٪) قاموسان**،
 *   والمستخدمُ يقرأ **واحداً**. والآليّةُ موجودةٌ أصلاً (`info.loader`
 *   بـ`src/i18n/index.ts` تحمّل السورانيةَ كسولاً) — فنقلُ الإنكليزية إليها
 *   يحرّر ~٦٦ كيلو ويسمح بإنزال هذا السقف لا رفعِه ثانيةً.
 * حتى ذلك الحين: **من يضيف نصّاً يقيس، ومن يضيف `import` يُرفض**. */
const BUDGET = { store: 175_000, main: 482_000 };
